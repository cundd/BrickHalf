...
std::pair<std::string, std::string> MapParser::splitToPair(const std::string &rawPair, char delimiter)
{
	size_t position = rawPair.find(delimiter);
	if (position != std::string::npos) {
		return std::make_pair(
				rawPair.substr(0, position),
...
std::vector<std::string> MapParser::splitToVector(const std::string &string, char delimiter)
{
